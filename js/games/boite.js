...
export function openBoiteReadOnly() {
  ...
  if (c.sponsor_logo) { const el = document.getElementById('boite-sponsor-logo'); el.textContent = ''; const img = document.createElement('img'); img.src = c.sponsor_logo; img.setAttribute('style','max-height:50px;max-width:150px;object-fit:contain'); img.onerror = () => { img.remove(); }; el.appendChild(img); }
