...
async function _submitPhotoMystere(chosen, stage, isCorrect, c) {
  ...
  if (demoMode) {
    ...
    const { data: p, error } = await db.from('users')
      .update({ xp: profile.xp + xpGain, coins: (profile.coins||0) + xpGain, interactions: (profile.interactions||0) + 1, photo_mystere_date: today })
      .eq('id', profile.id).select().single();
    if (error) { pmAnswered = false; showNotif('Oups, réponse non enregistrée.'); return; }
    setProfile(p);
    result = { correct: isCorrect, xp_gain: xpGain, answer: c.answer };
  }
